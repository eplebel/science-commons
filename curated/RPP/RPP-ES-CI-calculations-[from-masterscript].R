...
N.o <- MASTER$T_N..O.
N.r <- MASTER$T_N..R.
ES.data.frame <- cbind(id,ri.o,N.o,ri.r,N.r)
ES.data.frame.completed <- as.data.frame(subset(ES.data.frame,ri.r!="NA"))

#orig ES +/- CIs
Model.CIs<-res(r=ES.data.frame.completed$ri.o,n=ES.data.frame.completed$N.o)
Model.CIs
CIs<-round(((Model.CIs$u.r-Model.CIs$l.r)/2),digits=2)
#concatenate and remove leading & trailing white spaces;
ori.ES.CIs<-paste("r(N=",Model.CIs$N.total,") = ",sub('^(-)?0[.]', '\\1.', Model.CIs$r)," \u00B1 ",sub('^(-)?0[.]', '\\1.', CIs),sep="")
#write-to-file; CAUTION!!! use "RPP.study.number" column to match up with master file
#cat(iconv(ES.CIs, to="UTF-8"), file="_ori.ES.with.CIs.csv",append=TRUE,sep="\n")

#rep ES +/- CIs
Model.CIs<-res(r=ES.data.frame.completed$ri.r,n=ES.data.frame.completed$N.r)
Model.CIs
CIs<-round(((Model.CIs$u.r-Model.CIs$l.r)/2),digits=2)
#concatenate and remove leading & trailing white spaces;
rep.ES.CIs<-paste("r(N=",Model.CIs$N.total,") = ",sub('^(-)?0[.]', '\\1.', Model.CIs$r)," \u00B1 ",sub('^(-)?0[.]', '\\1.', CIs),sep="")
#write-to-file; CAUTION!!! use "RPP.study.number" column to match up with master file
#cat(iconv(ES.CIs, to="UTF-8"), file="_rep.ES.with.CIs.csv",append=TRUE,sep="\n")

combined.table <- as.data.frame(cbind(ES.data.frame.completed$id,ori.ES.CIs,rep.ES.CIs))
write.csv(combined.table, file="_ori.rep.ESs.CIs.csv", fileEncoding="UTF-8")





#rest of their code NOT USED
